...
export async function runPlanner(opts: PlannerOptions): Promise<PlannerResult> {
  ...
  const userPrompt = [
    ...
    '       Emit when the directive carries a `maxUsdPerTask` cap so the pool can',
    '       reject the task pre-launch when the estimate exceeds the cap.',
    '       Reasonable baselines: scaffolder $0.30-1.50, builder $0.20-1.00, fixer $0.10-0.50.',
    '',
