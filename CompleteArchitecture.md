...
## 22. Pluggable runtimes (Phase 10, added post-scaffold)

The assessor (`@factory5/assessor`) became language-pluggable in Phase 10, shipped as four runtime modules under `packages/assessor/src/runtimes/{python,node,go,rust}.ts` that all implement the same `RuntimeAssessor` contract from [ADR 0026](docs/decisions/0026-pluggable-runtime-contract.md). The dispatch table in `assess.ts` maps `AssessOptions.runtime` (one of `'python' | 'node' | 'go' | 'rust'`) to the registered module; `assess()` does the runtime-neutral artifact / module / git checks itself, then delegates the runtime-specific build + test slice to the chosen runtime's `runGate(projectPath, opts)`.

The contract has four sub-decisions:

1. **Provisioner shape — two flavors.** Python is _env-owning_: ADR 0017's `uv`-style venv layer creates the project's environment, installs deps, runs the gate. Node / Go / Rust are _env-assuming_: the project's own `package.json` / `go.mod` / `Cargo.toml` is the env; the runtime just runs commands against it. Node still does a `pnpm install --frozen-lockfile` preflight (because deps drive build outcomes), but it does not own the env.
2. **Verify-gate command mapping per runtime.** Python: `pip install -e . → pytest`. Node: `pnpm install --frozen-lockfile → pnpm typecheck (or tsc --noEmit) → pnpm test`. Go: `go build ./... → go test -list '.*' ./... → go test -v -count=1 ./...` (the `-v` is required because default `go test` output is package-level only; `-count=1` bypasses Go's test cache so the assessor always observes fresh subprocess output). Rust: `cargo test` (single command for build + test).
3. **Failure-mode taxonomy.** A new optional `AssessResult.failureMode` field tags the primary cause when a gate fails: `ENV_HOST_MISSING_TOOL` (host lacks a declared interpreter / package-manager / toolchain), `ENV_SETUP_FAILURE` (provisioning / preflight exited non-zero), `BUILD_FAILURE` (source did not compile / typecheck / import), `TEST_FAILURE` (build succeeded but tests failed or errored). Absent on green gates. Artifact-only failures (missing README, dirty git tree) flip `gate.verify` but do **not** set `failureMode` — they are completeness checks on the deliverable, not assessor failure modes.
4. **Host-tool pre-flight.** Each runtime declares its `hostTools: HostTool[]` (e.g. Node declares `node` + `pnpm`, Go declares `go`, Rust declares `cargo`). `assess()` probes each via `resolveOnPath` before any project-subprocess spawn attempt; the first missing tool short-circuits with `ENV_HOST_MISSING_TOOL` and an actionable install hint (e.g. "Install Go 1.21+ from <https://go.dev/dl/>").

The language a project gets gated against is set per-directive: the CLI's `factory build <project> --language <lang>` flag puts `language` into `directive.payload`, the brain's `extractRuntime` (in `loop.ts`) reads it back and passes it to `assess({runtime})`. The flag is sticky across `factory resume`. To avoid having to re-pass `--language` every build, `factory init <project> --language <lang>` writes the choice into `<project>/.factory/project.json` `metadata.language`, which `factory build` reads as a fallback when the explicit flag is absent. Default language is `python` — preserving back-compat with the existing `factory5-workspace/` Python corpus.

Phase 10 closed with all three new runtimes verified against real specs in autonomous mode: a TypeScript NDJSON CLI (Node, 14 vitest), a Go directory walker (Go, 34 go-test), a Rust CSV summarizer (Rust, 7 cargo-test). All three reached `gate.verify = true`. See `docs/Phase10_Progress.md` for the full arc, including the four pre-existing bugs that only the live runs could surface.

---

## 23. Web UI mutation surface (Phase 11, added post-scaffold)

Phase 9 (§21) shipped the Web UI's read-only surface; Phase 11 closed the loop with three mutation routes that turn the dashboard from a viewer into a complete operating surface. Operators can now answer pending questions, kick off builds, and configure per-project budget defaults from the browser without dropping back to the CLI. The contract is pinned by [ADR 0027](docs/decisions/0027-web-ui-mutation-surface.md):

| Route                           | Verb | URL                                    | Body                                         | Idempotency                                                                                                                          |
| ------------------------------- | ---- | -------------------------------------- | -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| Answer a pending question       | POST | `/api/v1/pending-questions/:id/answer` | `{ answer: string }`                         | Same-payload 200 no-op; different-payload 409 `QUESTION_ALREADY_ANSWERED_DIFFERENTLY` (first writer wins, never silently overwrites) |
| Kick off a build                | POST | `/api/v1/builds`                       | `{ project, language?, autonomy?, limits? }` | Not idempotent — each POST mints a new directive; SPA disables submit on first click                                                 |
| Set per-project budget defaults | PUT  | `/api/v1/projects/:id/budget`          | `{ maxUsd?, maxSteps? }`                     | Naturally idempotent — full RFC-9110 PUT (body is the new state of the document)                                                     |

All three reuse the existing `ipcErrorSchema` envelope and the same `FACTORY5_UI_TOKEN` bearer + loopback-only auth as the read-side (no weaker mutation check; CSRF out of scope per ADR 0025's same-origin design). Four new mutation-specific error codes (`QUESTION_ALREADY_ANSWERED_DIFFERENTLY`, `PROJECT_NOT_FOUND`, `PROJECT_PATH_UNREADABLE`, `PROJECT_METADATA_CORRUPT`); other failures (empty answer, invalid language enum, negative budget) collapse into `SCHEMA_VALIDATION_FAILED` with the field path in `details`.

The budget route's `metadata.budgetDefaults` lives under [ADR 0021](docs/decisions/0021-first-class-project-identity.md)'s `metadata: Record<string, unknown>` extension point — same slot as Phase 10's `metadata.language` — so no schema migration. Budget resolution gains a **project tier** between the existing CLI flag and instance config tiers: `--max-* flag → project.json metadata.budgetDefaults → ~/.factory5/config.toml [budget.defaults] → unlimited`. Per-field independent, so passing `--max-usd 5` does not flush a project's stored `maxSteps`. The same three-tier resolver runs in both `factory build` (CLI) and `POST /api/v1/builds` (web); Telegram / Discord inbound `/build` still skip it (I009 carry-forward — to be fixed by extracting a shared `resolveDirectiveLimits(projectMeta, cfg, explicitFlags)`).

The SPA wires three forms through a centralised `apps/factory-web/src/lib/api.ts` that adds `apiPost<TReq, TRes>` / `apiPut<TReq, TRes>` wrappers around the existing `apiFetch` envelope unwrap. Forms switch on the pinned error-code list to render route-specific UI (e.g. show the recorded answer when 409 fires on answer; navigate to the directive detail page when 200 fires on build). Two new SPA pages landed: `pages/projects/{index,detail}.astro` and `pages/build.astro`; the existing questions detail page got the answer textarea retrofit. Two read-side prerequisites landed in the same step: `GET /api/v1/projects` (list — feeds the build form's project dropdown) and `GET /api/v1/projects/:id` (detail — feeds the budget form, returns `budgetDefaults` + `language` extracted from the on-disk `project.json` so the SPA does not walk the free-form metadata blob client-side).

Phase 11 closed 2026-04-26 with operator-driven browser smoke against a real factoryd: full assisted-mode build (`log-totals-cli`, $4.25, scaffolder + 3 builders + verifier) plus two mid-stream answers via the SPA form plus a project-tier budget propagation check (Build #1 `hasLimits:false` vs Build #2 `hasLimits:true` after a PUT, same form, no body limits). See `docs/Phase11_Progress.md` for the full arc.

---

## 24. Worker filesystem-scoping (Phase 12, added post-scaffold)

Phase 7's tool-using worker subprocess shipped with `allowedTools: ['Read','Write','Edit','Bash','Glob','Grep']` and `--dangerously-skip-permissions` — host-wide filesystem access. Phase 12 closed that surface. The contract is pinned by [ADR 0028](docs/decisions/0028-worker-sandbox-contract.md): three Claude Code-native primitives layered per-spawn at the worker boundary, no monkey-patching, no OS sandbox, no custom MCP middleware (a pre-decision survey confirmed Claude Code's MCP layer cannot intercept built-in `Read`/`Write`/`Edit`/`Bash`/`Glob`/`Grep` tools — it can only add new MCP-namespaced tools).

Three forcing functions converged here: F001 (Phase 6c verifier hallucination — its model context included irrelevant repo-internal files; ADR 0018 demoted it to advisory as an interim), Phase 8 carry-forward (ADR 0024 explicitly deferred fs scoping), and Phase 10 I013 (worktree-cleanup pain on Windows because workers polluted worktrees with `node_modules/` from outside). Phase 12 paid down all three with one path-prefix gate.

The gate composition:

| Layer                 | Primitive                                                      | What it gates                                                                                                                            |
| --------------------- | -------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| Static deny           | `permissions.deny` in `<worktree>/.claude/settings.local.json` | Coarse blocks on obvious danger zones: `Read(~/.ssh/**)`, `Read(//etc/**)`, `Read(C:/Windows/**)`, `Edit(~/**)`, …                       |
| Affirmative allowlist | `PreToolUse` hook (small Node script, JSON-over-stdin)         | Per-call path-prefix algebra: `tool_input.file_path` resolved → absolute, checked against `workspaceRoots ∪ readOnlyRoots` per asymmetry |
| Mode                  | `--permission-mode acceptEdits` (replaces `bypassPermissions`) | Auto-accepts edits within `cwd ∪ additionalDirectories`; eliminates the host-wide bypass `--dangerously-skip-permissions` carried        |

Per Claude Code's precedence model: deny rules > hook decisions > allow rules. The static-deny layer is absolute; the hook handles the affirmative algebra (which deny patterns can't cleanly express because the worktree path is per-task); `acceptEdits` + `additionalDirectories` provide mode-level auto-accept for edits within scope. Defence-in-depth: a misconfigured hook still denies the obvious danger zones via static rules.

The `WorkerSandboxConfig` shape is `{ workspaceRoots: string[]; readOnlyRoots: string[]; allowSymlinks: boolean }`. Phase 12 defaults set `workspaceRoots: [worktree.path]` (the per-task worktree, the only writeable root), `readOnlyRoots: [<projectPath>/.factory, <repoTemplatesDir>]` (so workers can read prior plans / findings / `project.json` / templates without writing outside the worktree), `allowSymlinks: false` (symlinks rejected without dereferencing — `lstatSync`-checked).

Write-vs-read asymmetry is explicit in the gate function. Write-class tools (`Write`, `Edit`) require `pathInsideAny(absolutePath, workspaceRoots)`. Read-class tools (`Read`, `Glob`, `Grep`) require `pathInsideAny(absolutePath, workspaceRoots ∪ readOnlyRoots)`. `Bash` is intentionally **not** matched by the hook (Bash is shell-shaped, not fs-shaped; arbitrary command parsing is unsound). Bash is gated solely by cwd pinning to the worktree + a small set of static command-pattern denies (heuristic, leaky); real OS-level Bash isolation is deferred to a follow-up phase.

Out-of-scope tool calls receive a hard-deny (`permissionDecision: "deny"`) with a reason naming the allowed roots — deliberately not the deny rules, so the LLM gets adaptation hints without evasion-friendly information. Audit lines stream to stderr per call (`factory5.worker.sandbox {"event":"sandbox.gate","tool":"Read","decision":"allow",…}`).

The cross-platform algebra (`pathInsideAny` + `normaliseForCompare`) handles Windows case-insensitive prefix, separator normalisation (`\` ↔ `/`), drive-letter case, UNC paths, `..` traversal, and trailing-slash insensitivity — covered by 89 unit tests in `@factory5/worker-sandbox` (with `describe.skipIf` gating Windows-only and Linux/macOS-only assertions).

Implementation lives in the new 15th workspace package `@factory5/worker-sandbox`. Public API: `WorkerSandboxConfig`, `evaluateToolCall`, `pathInsideAny`, `writeWorktreeSandbox`, `getHookScriptPath`, plus the pure `runHook` runtime that the `dist/hook-runtime.js` script wraps. The worker (`packages/worker/src/run-worker.ts`) calls `prepareSandbox` between worktree allocation and `provider.stream`, switches `permissionMode` from `'bypassPermissions'` to `'acceptEdits'`, and `rm -rf`s the per-spawn `<worktree>/.claude` directory in finally so the merge-back commit isn't polluted. `ProviderRequest` was deliberately **not** extended with a `sandbox?` field — keeping the gate worker-side means `@factory5/providers` stays LLM-agnostic; future runtimes that adopt other CLIs translate their own gates.

Reversibility is layered: `FACTORY5_DISABLE_WORKER_SANDBOX=1` env var skips `prepareSandbox` and falls back to `bypassPermissions` (operator escape hatch); removing the package + reverting the worker hook returns to host-wide fs access; loosening the algebra (e.g. `allowSymlinks: true`) is config-only.

Phase 12 closed 2026-04-26 with operator-driven `factory build log-totals-cli` end-to-end against a real factoryd: 5/5 pool tasks succeeded under the new gate (scaffolder + 3 builders + verifier), 4 `worker.sandbox: gate up` lines emitted, **zero deny lines** across the full build (the LLM never reached out of scope), worktree cleanup clean throughout (no `Directory not empty`), one builder demonstrated legitimate writes by advancing base from `aa3a1263 → 0d4dcbc3`, total live spend $3.07. See `docs/Phase12_Progress.md` for the full arc.
