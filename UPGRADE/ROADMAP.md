...
- [x] Per-project budget defaults extension — `<project>/.factory/project.json` `metadata.budgetDefaults` widens from `{maxUsd, maxSteps}` to all axes; three-tier resolution preserved
- [x] Per-task USD cap (`maxUsdPerTask`) — new seventh axis in `BUDGET_DEFAULTS`; pool pre-launch check; planner emits per-task `estimatedUsd`; auto-answer recognition generalises across axes
...
- [x] `runArchitectWithCritique` wrapper (brain, TDD)
- [x] loop integration + delete `wikiReadiness()` + delete old tests
...
- [x] Browser smoke verified live at `01KSAVBNVNTARM6EPFPPJFQZKT` (tier-14-smoke, 2026-05-23) — critic loop fires end-to-end; Sonnet architect ($0.110) + Opus critic ($0.172) = $0.282 wiki-phase spend; critic passed first try (no live retry — covered by 42 unit tests across critic/architect-loop/auto-answer)
...
- [x] Brain: `pool-resume.ts` chokidar watcher (parked → raise → auto-resume)
- [x] Brain: pool-driven dispatcher rewrite (`pool.ts`) + planner stops emitting `task.maxTurns` + worker watchdog
