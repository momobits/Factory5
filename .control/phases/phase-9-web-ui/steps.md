...
- [ ] 9.2 — `apps/factory-web/` scaffold. New workspace app (not package) with its own `package.json`, `tsconfig.json`, dev script. Wire into pnpm-workspace via `apps/*` glob (no edit needed). Minimal "hello world" page served by the chosen framework's dev server. Include the bearer-token gate scaffolding even in dev.
