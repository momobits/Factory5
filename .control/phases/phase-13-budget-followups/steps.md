...
- [ ] 13.5 — Per-project budget defaults extension. `@factory5/core` project metadata schema widens `metadata.budgetDefaults` from `{maxUsd?, maxSteps?}` to cover all axes from `BUDGET_DEFAULTS`. Daemon POST `/builds` body-resolution path (ADR 0027 §4) merges new keys per-axis. Tests cover the three-tier resolution chain for each new axis.
