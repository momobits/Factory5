...
- [x] Live browser smoke verified at `01KSAVBNVNTARM6EPFPPJFQZKT` (2026-05-23, tier-14-smoke): critic loop fires end-to-end via `/app/build` form; activity panel narrates `brain.architect-loop` evaluating + `brain.architect` calling `claude-sonnet-4-6` + `brain.critic` calling `claude-opus-4-7 (category reasoning)` + `brain.architect-loop` "passed on attempt 1" with full Opus verdict rendered. Sonnet-architect + Opus-critic wiki phase cost $0.282; comparable to pre-Tier-14 Opus-only architect. Live retry/exhaustion paths NOT exercised (critic passed first try even on the deliberately-thin CLAUDE.md — Sonnet still produced a 3-page wiki the Opus critic accepted) — coverage falls to the 16 architect-loop + 16 auto-answer + 10 critic unit tests. Persistence verified: `directive.payload.budgets.maxWikiReadinessAttempts: 3` round-tripped via API GET.
