...
- [x] 11.7 — `/phase-close` — verified all 10 done-criteria; tagged `phase-11-directive-log-persistence-closed`; appended session entry to `UPGRADE/LOG.md`; transitioned STATE → Phase 12 active (budget-ux, already scaffolded). Live browser smoke (Playwright MCP, smoke-demo project, $1.00 cap, real spend $0.6238): refresh on running directive restored activity panel from `/logs`; tab 2 opened mid-build showed identical 7 events to tab 1; terminal directive reloaded after `blocked: budget_exceeded_usd` showed full 9-event narrative (triage → architect ×3 → planner ×2 → pool ×2 → loop).
