...
- [x] 10.2 — ADR 0031: log-forwarder design — pin manual `emitLogLine` sites as first-ship; pino-transport-tap and hybrid listed as alternatives considered. Update `docs/decisions/INDEX.md`.
- [x] 10.3 — Brain emit sites — add `emitLogLine` calls in `architect.ts`, `planner.ts`, `pool.ts`, `loop.ts` per the plan's table. `planner.ts:331` parse-fail and `:335` Zod-fail both emit `error`-level lines with first 500 chars of LLM response in `attrs.detail`. Extend `loop.test.ts` with happy-path emission assertions + a regression test for the planner parse-fail surfacing the error line.
- [x] 10.4 — Daemon `POST /api/v1/directives/:id/resume` — mirror `packages/cli/src/commands/resume.ts` logic over HTTP. New `apiV1ResumeRequestSchema` + `apiV1ResumeResponseSchema` in `packages/ipc/src/`. Bearer-auth + Zod body validation pattern from `/api/v1/builds`. 404 on missing prior; 409 on prior `running`/`pending`; 422 on prior `projectPath` not on disk. Integration test in `packages/daemon/src/server.test.ts` mints a prior, POSTs to resume, asserts `parentDirectiveId` + `payload.resumeFrom` chain on the child + doorbell emission.
- [x] 10.5 — UI Resume button — `apps/factory-web/src/pages/directives/detail.astro` renders a Resume button next to the title when `effectiveStatus()` ∈ `failed | blocked | complete` AND `intent === 'build'`, parallel to the existing Cancel control. `apps/factory-web/src/pages/projects/index.astro` gains a "Last build" column with the latest build directive's status (linked) + a per-row "Resume" pill button on terminal directives. On-click POSTs to `/api/v1/directives/<id>/resume`; on 2xx navigates to the new directive's detail page. Resume + cancel + row-resume styles live in `Dashboard.astro`'s global `<style is:global>` block (Astro scoped rules don't reach JS-created elements — same gap also fixed the pre-existing unstyled `.cancel-btn` for runtime use).
- [x] 10.6 — UI activity panel refinements — level badges (info acid / warn amber / error halt-red as outlined pills using Tier 9 design tokens); empty-state "Waiting for the brain to narrate…" when zero `log.line` events have arrived on a `running`/`pending` directive (silent on terminal directives with no events); existing auto-scroll-pin preserved (Resume-tailing button renamed `log-resume-tail-btn` to avoid colliding with the title-row directive-resume button; both styles moved to `Dashboard.astro`'s `<style is:global>` block so they reach JS-created elements). Header renamed "Live log" → "Activity"; counter renamed "lines" → "events" to match the SSE event vocabulary.
- [x] 10.7 — `/phase-close` — verify all done-criteria checkboxes flip; tag `phase-10-resume-and-activity-feed-closed`; append final session entry to `UPGRADE/LOG.md`; transition STATE back to "all phases complete" (seventh time) unless a Tier 11 demand signal arrives. Live browser smoke completed this session via Playwright MCP — Resume click minted child directive `01KRR9RGFN10YMDX5C16TXK91Y`, activity panel narrated 5 events end-to-end. U030 closed.
