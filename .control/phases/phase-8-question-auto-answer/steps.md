...
- [x] 8.2 — Migration 009 — `pending_questions.answered_by` column + backfill (orphan-sweep rows from `[orphaned by ...]` prefix; all other answered rows → `'user'`); update `pendingQuestionSchema` (`@factory5/core`) with optional `answeredBy` field; extend `pendingQuestions.answer` to accept `answeredBy` parameter (default `'user'`, race-loser-safe via `WHERE answered_by IS NULL`); update `markOrphanAnswered` to set `'orphan-sweep'` going forward
- [x] 8.3 — ADR 0030 — pending-question auto-answer contract: pins `answered_by` enum, deadline-config home (`<dataDir>/config.json`), LLM dispatcher failure semantics, race mitigation, spend treatment, no-override-after-auto-answer rule; update `INDEX.md`. ADR 0024 cross-reference dropped per CLAUDE.md "do not edit accepted ADRs" — ADR 0030 references 0024 in its own header instead
- [x] 8.4 — `loadConfig()` / `writeConfig()` reader/writer for `<dataDir>/config.json` (Zod-validated; missing file returns defaults; partial file fills missing keys; atomic tmp+rename writes that merge with existing keys). Schema + `DEFAULT_ASK_USER_DEADLINE_MS = 300_000` exported from `@factory5/core` (pure schema/types); I/O lives in `@factory5/state/src/config.ts` (joins `dataDir()` for path resolution alongside `defaultDbPath`). Plan-deviation: state vs core for I/O — keeps core free of fs operations. 11 unit tests
- [x] 8.5 — Brain stamps `deadline_at` from `loadConfig().askUserDeadlineMs` on every new `ask_user` pending-question. Audit found a single production call site (`packages/brain/src/ask-user.ts:213`); the daemon-side `askUser({...})` invocation at `packages/daemon/src/index.ts:605` flows through it. Caller-provided `deadlineAt` wins; absent → auto-stamped from cached config (`resetDeadlineCache()` exposed for tests). Logged-fallback on corrupt config so brain doesn't crash on every emission. 3 new tests added (auto-stamp from config, default fallback when file absent, caller override wins)
- [x] 8.6 — Brain auto-answer dispatcher + deadline sweep: new `findOpenPastDeadline(db, now, limit?)` + `claimForAutoAnswer(db, id, when)` + `finalizeAutoAnswer(db, id, answer, when, answeredBy)` queries in `pending-questions.ts`; new `packages/brain/src/auto-answer.ts` builds prompt (question + options + parent directive + past Q&A in this directive) → dispatches via existing model/provider with one retry → writes `answered_by = 'agent'` on success / `'agent-failed'` on retry-then-fail; sentinel race-mitigation claim before LLM call; spend recorded against parent directive on success only (ADR 0030 §6); wired into `runServe` loop with 5s throttle (`AUTO_ANSWER_SWEEP_INTERVAL_MS`). 10 tests in `auto-answer.test.ts` (happy path, retry-then-success, retry-then-failed, empty-then-failed, claim-lost no-op, batch sweep ordering, terminal-directive skip, no-rows no-op, 2 prompt-shape tests). Plan-deviation: dropped CLAUDE.md/task_log/findings from prompt context for first ship — generic prompt covers question + options + directive + past Q&A; agent-class-specialized prompts deferred per ADR 0030 alternatives section
- [x] 8.7 — Surface updates: web `/app/questions/index` adds an "Answered by" column rendering 'human' / 'agent (auto)' / 'agent (LLM failed)' / 'orphan sweep'; `/app/questions/detail` adds an "Answered by" meta row (only shown on answered rows). CLI `factory questions list / show <id>` deferred — those subcommands don't exist today; only `factory questions cleanup` exists, and it operates on orphan rows where `answered_by IS NULL` until the sweep claims them. Closes U029
- [x] 8.close — `/phase-close` — tag `phase-8-question-auto-answer-closed`; append session entry to [`../../../UPGRADE/LOG.md`](../../../UPGRADE/LOG.md); upgrade arc re-closed (no Phase 9 plan authored — fifth time)
