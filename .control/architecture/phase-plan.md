...
| 11  | directive-log-persistence | phase 10 (consumes its SSE log.line emission contract) | ~1 | Migration 010 `directive_log_lines` table; daemon `DirectiveStreamHub.emit` tees `log.line` events to DB before fanning out; new `GET /api/v1/directives/:id/logs?since=<iso>` route; FE directive-detail replays historic on connect + dedups against live SSE stream. Activity panel now survives reload + multi-tab consistency. Closes U031 |
| 12  | budget-ux | phase 11 (sequential; both audit-driven follow-ups to Tier 10) | ~2 | Surface every operator-felt budget at build time (Web UI Build form accordion + CLI flags) with defaults + explainers from a single `BUDGET_DEFAULTS` source in `@factory5/core`. Persist budgets in directive payload; resume inheritance. Brain escalation on `error_max_turns` via typed askUser ("Task X out of turns at 80; bump to 120?") instead of hard-fail; Tier 8 auto-answer adapts (first-failure bump, second-failure abort). ADR 0032 pins the budget UX paradigm. Closes U032 |
| 13  | budget-followups | phase 12 (fixes the propagation gap surfaced by Phase 12's deferred smoke) | ~2-3 | Close the operator-felt budget loop Phase 12 built but couldn't demonstrate end-to-end. Fix U033 — `resolveTaskMaxTurns` returns `min(planner_emit, operator_ceiling)` so the operator's `payload.budgets[axis]` actually floors the planner's per-task emit. Fix U034 — `factory daemon stop` on Windows leaves no stale pidfile (CLI-side belt-and-suspenders post-`waitPidGone`). Extend per-project `metadata.budgetDefaults` to cover all six axes from Phase 12. Add seventh `maxUsdPerTask` axis with pre-launch escalation. Auto-answer's `[BUDGET]` recognition refactors from `maxTurns*`-coupled to axis-agnostic. ADR 0032 amendment (default) or new ADR 0033 (only if paradigm shifts). Closes U033 + U034 |
| 14  | wiki-readiness-judge | phase 13 (reopens arc; independent code) | ~2-3 | Replace the over-literal regex wiki-readiness gate with an LLM judge that evaluates against directive intent, iterates the architect with critique feedback, and escalates to the operator on exhaustion (ADR 0033). New `runWikiCritic` + `runArchitectWithCritique` wrapper; architect default category flips from `reasoning` (Opus) to `planning` (Sonnet); new `critic` agent role (10th); 8th `BUDGET_DEFAULTS` axis (`maxWikiReadinessAttempts`); new `[agents.*]` config table. Closes U035 |
| 15  | budget-ux-overhaul | phase 14 (reopens arc; independent code) | ~2-3 | Phase 15 replaces the `[BUDGET]` askUser with a project-level budget cockpit, switching `maxTurns*` axes to directive-wide pools and adding an optional auto-increase policy (ADR 0034). |

Phases 3 and 4 share no critical code — order is operator preference once Phase 2 closes. Phase 5 has no code dependencies (pure docs + prompts + an optional small CLI command); shippable after Phase 4 closes (the natural sequence) or independently. Phase 6 is the natural continuation of Phase 5 — closes the agent-prompts arc by auditing the skills the prompts cite + wiring the fixer marker grammar Phase 5 documented. Phase 7 is the operator-side composition of Phase 6's agent-side parser — single CLI verb that wraps the same `updateFindingStatus` call the parser dispatches. Phase 8 reopens the arc post-Phase-7-close with the highest-leverage carry-forward — autonomous-run unblock when the human is absent. Phase 9 reopens the arc again for a frontend aesthetic overhaul — the first tier in the arc that ships visual-design work without an underlying contract change; informal cadence (single change set, no per-step commits) at operator's request. Phase 10 reopens the arc again for two operator-feels-blind gaps surfaced by an `automl` build failure: no UI resume surface; directive-detail activity panel silent because the brain emits only one `log.line` event today. Phase 13 reopens the arc once more for the propagation gap Phase 12's deferred smoke surfaced: structural budget UX intact but the operator-set value never reaches the worker because the planner emits a per-task value that always shadows the operator's directive budget. Phase 14 replaces the regex wiki-readiness gate with an LLM judge that evaluates against directive intent, iterates the architect with critique feedback, and escalates to the operator on exhaustion (ADR 0033). Phase 15 reopens the arc again to eliminate the `[BUDGET]` askUser paradigm — an incident during a live build showed the parser rejects natural-language replies that any human would recognize as valid, cascading 12 task failures; the root cause is the structured-reply paradigm itself, not the parser (ADR 0034).
...
**Goal:** persist `log.line` SSE events to SQLite so the directive-detail activity panel survives page reloads and is consistent across tabs. Today the events are ephemeral — emitted by the brain, broadcast by the SSE hub, dropped when the subscriber disconnects. Operators reopening a terminal directive see an empty panel; tabs that subscribe after another tab miss the historic events. Three operator-felt failure modes surfaced 2026-05-16 (refresh forgets everything; multi-tab event split; post-mortem invisibility on a `failed` directive).

**Key steps:** open U031; migration 010 (`directive_log_lines` table with directive_id / ts / level / component / msg / attrs_json columns + ts index); state queries (`appendLogLine`, `listForDirective`); daemon `DirectiveStreamHub.emit` tees `log.line` events to DB before fanning out (synchronous insert; failure logs `warn` and continues — emission must not block on persistence); new `GET /api/v1/directives/:id/logs?since=<iso>&limit=<n>` route with bearer-auth; FE replays historic logs on directive-detail load before attaching SSE, with a join-cursor dedup against the live stream.

Full plan: [`../../UPGRADE/plans/tier-11-directive-log-persistence.md`](../../UPGRADE/plans/tier-11-directive-log-persistence.md). Issues addressed: U031.

**Done criteria highlights:** all four `pnpm` gates clean; migration 010 lands (three pre-existing migration shape tests bump to `[1..10]`); state queries + tests; daemon hub-tee integration test; route integration tests (401, since-cursor filter, happy path); FE replay + dedup; browser smoke proves refresh-survives, multi-tab consistency, terminal-post-mortem visibility.

### Phase 12 — budget-ux

**Goal:** stop the app from silently picking budget defaults the operator never sees, and stop the app from hard-failing tasks when a budget can be cheaply extended. Surface every operator-felt budget at build time with defaults + explainers; persist them on the directive; inherit on resume; escalate on `error_max_turns` via typed askUser instead of hard-fail. Driving operator complaint 2026-05-16: *"why are we failing instead of asking the user if we should continue over the budget? why have a max cost and max steps that we ask the user and have other limits the user does not see?"* — fifteen hardcoded budgets audited in `UPGRADE/plans/tier-12-budget-ux.md`, six identified as operator-facing.

**Key steps:** open U032; ADR 0032 (budget UX paradigm: operator-facing vs internal-pacing budgets, default-publication contract, escalation rule, persistence contract); new `BUDGET_DEFAULTS` constant + Zod schema in `@factory5/core` as single source of truth for defaults + explainers; Web UI Build form gains "Advanced budgets" accordion with six fields (maxUsd, maxSteps, askUserDeadlineMs, maxTurnsScaffolder/Builder/Fixer); CLI flags + `--help` text quoting the same explainers; directive `payload.budgets` field; Tier 10 resume route inherits the full budget set; brain `pool.ts` detects `error_max_turns` subtype, raises typed askUser ("Task X out of turns at 80; bump to 120?"), relaunches task on accept; Tier 8 auto-answer adapter recognises the budget-escalation prompt and applies bump-by-one-bucket on first failure / abort on second.

Full plan: [`../../UPGRADE/plans/tier-12-budget-ux.md`](../../UPGRADE/plans/tier-12-budget-ux.md). Issues addressed: U032.

**Done criteria highlights:** all four `pnpm` gates clean; ADR 0032 lands; CLI + Web + project-metadata parsers all read `BUDGET_DEFAULTS`; resume inheritance covers the full budget set; brain escalation tests cover both accept-bump and abort paths; browser smoke confirms a budget-tripping task escalates via askUser rather than hard-failing.

### Phase 13 — budget-followups

**Goal:** close the operator-felt budget loop Phase 12 structurally built but couldn't demonstrate end-to-end. Phase 12's smoke surfaced the deeper bug — `resolveTaskMaxTurns` prefers `task.maxTurns` (planner-emit, always set per the planner prompt's 10-160 range) over `directive.payload.budgets[axis]` (operator), so the operator's UI-set budget never reaches the worker. Plus polish (U034: Windows `factory daemon stop` leaves a stale pidfile because Node maps SIGTERM to TerminateProcess and the daemon's shutdown handler never runs) and two cheap Phase 12 carry-forwards (per-project default overrides extending to all axes; per-task USD cap).

**Key steps:** open U033 + U034 in ROADMAP (already filed in ISSUES.md); fix U033 — `resolveTaskMaxTurns` returns `min(planner_emit, operator_ceiling)` so the operator's `payload.budgets[axis]` actually floors the planner's per-task emit, docstring rewrites from "operator override" to "operator ceiling — planner emit refines downward," ADR 0032 amendment block (default) or new ADR 0033 (only if paradigm shifts meaningfully); fix U034 — post-`waitPidGone()` belt-and-suspenders in `packages/cli/src/commands/daemon.ts`, new `unlinkPidFile()` export, same-PID predicate to handle the race-restart edge case; extend `<project>/.factory/project.json` `metadata.budgetDefaults` schema in `@factory5/core` to cover all six axes (today only `maxUsd`+`maxSteps` from Tier 8), three-tier resolution chain (instance config → project metadata → body flags) preserved; ship `maxUsdPerTask` axis with planner-side `estimatedUsd` per task + pool pre-launch check + typed `[BUDGET]` askUser escalation reusing Phase 12's pattern; auto-answer `[BUDGET]` recognition refactors from `maxTurns*`-coupled to axis-agnostic; live browser smoke (Playwright MCP, `smoke-demo`, $1.50 cap) closes the Phase 12 deferred gate.

Full plan: [`../../UPGRADE/plans/tier-13-budget-followups.md`](../../UPGRADE/plans/tier-13-budget-followups.md). Issues addressed: U033, U034.

**Done criteria highlights:** all four `pnpm` gates clean; ADR 0032 amendment lands (or ADR 0033 supersedes); `resolveTaskMaxTurns` 5+ new tests covering ceiling / refine-down / unset / read-only-agent / edge-case-zero; cross-platform daemon-stop CLI integration test; project metadata schema accepts all axes; `BUDGET_DEFAULTS` gains `maxUsdPerTask` with pre-launch escalation; browser smoke confirms operator-set `maxTurnsScaffolder=10` actually floors the planner emit → `[BUDGET]` askUser fires → accept → retry → success.

### Phase 14 — wiki-readiness-judge

**Goal:** replace the over-literal regex wiki-readiness gate with an LLM judge that evaluates the wiki against the directive's intent + the project's CLAUDE.md + all wiki pages on disk. The judge produces a structured `WikiCritique`; on failure the architect re-runs with the critique as feedback; on exhaustion the brain files an `askUser` for operator decision (auto-answer defaults to `continue` per the new `[CRITIC]` marker case in ADR 0030). Driving issue U035: the `checkModules` regex fires on most builds because Opus frequently produces `# Modules` H1, `## Components`, or other shapes where the regex expects `\n## Modules` H2.

**Key steps:** open U035; ADR 0033 (new, six-part decision) + amendment blocks on ADR 0032/0004/0030; new `wikiCritiqueSchema` + `AGENT_ROLES` += `'critic'` + `BUDGET_DEFAULTS` 8th axis (`maxWikiReadinessAttempts`, default 3) in `@factory5/core`; `agentsConfigSchema` + `resolveAgentCategory` + `DEFAULT_AGENT_CATEGORIES` in `@factory5/state`; new `runWikiCritic` module in brain (single LLM call, `brain.critic` log lines); `runArchitect` grows `priorCritique?` param + resolves category via config (default flips from `reasoning` to `planning`); new `runArchitectWithCritique` wrapper orchestrates retry loop + `askUser` exhaustion; `loop.ts` swaps to wrapper; `wikiReadiness()` + helpers + tests deleted; daemon/CLI/Web UI accept the 8th axis; live browser smoke closes the phase.

Full plan: [`../../UPGRADE/plans/tier-14-wiki-readiness-judge.md`](../../UPGRADE/plans/tier-14-wiki-readiness-judge.md). Issues addressed: U035.

**Done criteria highlights:** all four `pnpm` gates clean; ADR 0033 lands; `wikiReadiness()` and its 4 helpers deleted with no remaining importers; `runWikiCritic` + `runArchitectWithCritique` + `runArchitect priorCritique` parameter all tested per spec §8; `BUDGET_DEFAULTS` has 8 axes with `maxWikiReadinessAttempts` flowing through CLI + Web UI + per-project + payload + resume; `AGENT_ROLES` has 10 entries; live browser smoke shows critic fires on a thin-wiki project with at least one retry observed and a distinct `critic` row in the spend rollup; workspace test count ≥ 1340 passing.

### Phase 15 — budget-ux-overhaul

**Goal:** replace the entire `[BUDGET]` askUser escalation path with a project-level budget cockpit. Switch the three `maxTurns*` axes from per-task caps to per-agent-class directive-wide pools. Live re-resolve from `project.json` so the operator can raise a cap while a directive is running without restarting it. Optional auto-increase toggle bounded by a safety multiplier ceiling. The parser, the askUser, the structured-options-UI gap, and the per-axis bucket schedule all disappear together.

**Key steps:** open U036 + U037 + U038 in ISSUES.md; ADR 0034 (new, supersedes ADR 0032) + amendment blocks on ADR 0030 (delete `[BUDGET]` branch) + ADR 0020 (pool semantics cross-ref); new `autoIncreaseBudgets` + `autoIncreaseCeilingMultiplier` scalars in `projectMetadataSchema`; new `computePoolUsage(db, directiveId, projectBudgets)` helper aggregating task turn counts + model_usage USD per directive; new `pool-resume.ts` chokidar watcher flips parked directives back to running on `project.json` mutation when cap has headroom; `pool.ts` dispatcher rewrite replaces per-task retry-loop with pool-driven pre-launch check + worker watchdog; `budget-escalation.ts` deleted (~360 lines) + `[BUDGET]` branch removed from `auto-answer.ts`; daemon extends `PUT /budget-defaults` to 8 axes + 2 scalars, adds `GET /pool-usage` route, emits `pool.tally` SSE event; `apps/factory-web/src/pages/projects/detail.astro` full rewrite into four-tab cockpit (Live / Defaults / History / Settings); directive detail page adds pool pill; build form accordion copy updated.

Full plan: [`../../UPGRADE/plans/tier-15-budget-ux-overhaul.md`](../../UPGRADE/plans/tier-15-budget-ux-overhaul.md). Issues addressed: U036, U037.

**Done criteria highlights:** all four `pnpm` gates clean; ADR 0034 lands; `budget-escalation.ts` deleted with no remaining importers; `[BUDGET]` branch gone from `auto-answer.ts`; `pool-usage.ts` / `pool-resume.ts` / `pool.ts` all tested; `GET /pool-usage` route integration tests pass; `pool.tally` SSE event fires; project detail page four tabs render; parked-alert banner + auto-resume CTA work end-to-end; browser smoke #1 (parked → raise → auto-resume) verified; U036 + U037 closed.
