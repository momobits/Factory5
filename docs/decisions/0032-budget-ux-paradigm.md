...
§1's closed-set axis table extends from seven to eight axes with the addition of `maxWikiReadinessAttempts` (default 3). Note: Phase 13.6 added `maxUsdPerTask` as the seventh axis but did not amend this ADR at the time; this Phase 14 amendment records both that the axis count is now eight and (retroactively) that `maxUsdPerTask` is the seventh per ADR 0032's source-of-truth contract. `maxWikiReadinessAttempts` is the architect+critic retry cap from ADR 0033's wiki-readiness critique loop. Zero sentinel = unlimited, matching `maxUsd: 0` / `maxSteps: 0`. §3 `BUDGET_DEFAULTS` source-of-truth contract unchanged; the table just grows. No supersedure per CLAUDE.md "do not edit accepted ADRs" — this is a contract-preserving extension, not a paradigm change (precedent: Phase 13.3's amendment for U033 clarification).
