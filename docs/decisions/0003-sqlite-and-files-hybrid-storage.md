...
**Negative:**
- Two storage models means two mental models. Mitigated by the rule "if it's about *one project*, it's a file; if it's about *the factory*, it's SQLite."
...
- **SQLite for everything** (project state in SQLite too). Rejected because it destroys the wiki philosophy — projects must remain portable and human-readable. The wiki shipping *with* the project as documentation is part of the value proposition.
