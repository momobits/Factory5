...
| Event                 | Verified live in 2026-05-05 smoke | Verification path                                                                                                                                                                                                  |
| --------------------- | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `task.started`        | ✅                                | 3 tasks each rendered "Tasks (N)" panel update in real time                                                                                                                                                        |
| `task.completed`      | ✅                                | Both successes and the verifier's failure rendered with terminal status                                                                                                                                            |
| `spend.updated`       | ✅                                | Spend pip ticked $0.46 → $0.54 → $1.24 across the build                                                                                                                                                            |
| `log.line`            | ✅                                | Worker subprocess activity rendered in the log tail                                                                                                                                                                |
| `finding.created`     | **Unit-tested only** (see below)  | 4 unit tests in `packages/brain/src/pool.test.ts` (added in commit `f990323` for 3.1b); live emission was not exercised because the smoke-demo project produced no findings (`findings_registry` empty post-build) |
| `directive.completed` | ✅                                | Operator-driven cancel from the FE flipped status; FE rendered the new state via the `directive.completed` round-trip                                                                                              |

**`finding.created` caveat.** The wire-side plumbing for `finding.created` is identical in shape to the four live-verified event types (same hub, same callback, same Zod schema, same FE consumer pattern). The brain-side emission code path (`emitFindingCreated` in `pool.ts`) is exercised by 4 unit tests covering: emitter-undefined-silent, well-formed-event-with-advisory-false-default, advisory-true-pass-through, one-event-per-call. The smoke didn't surface any findings because the directed `smoke-demo` project's worker tasks were intentionally simple ("scaffold + add(a,b) + test") — no verifier-class issues to register, no advisory findings emitted. The risk of a live regression in `finding.created` specifically (vs. the four verified types) is judged low given the parallel implementation shape; the phase-3 acceptance smoke at `/phase-close` will exercise it naturally on a more substantive build.
...
- **`finding.created` live verification gap (documented).** Per the Live verification section above; mitigation by unit tests + the parallel implementation shape with verified event types. Not a known-broken state — a known-not-yet-witnessed-live state.
...
- Brain-side `log.line` forwarder (selective pino-stream tap filtered by `correlationId`). Target: phase-3 acceptance follow-up or phase-4 carry-forward.
- Findings backfill on connect (currently fetched via separate `GET /api/v1/findings` route). Target: phase-3 acceptance follow-up if operator surfaces a need.
- Phase-3 acceptance smoke (`/phase-close`-time) exercises `finding.created` live on a substantive build, closing the live-verification gap recorded above.
