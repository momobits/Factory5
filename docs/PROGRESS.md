...
- **No new ADRs.** Sweep phase like Phase 13. None of the four fixes warranted pinning a new contract — 14.1 stayed inside the package.json/launcher surface (ADR 0029 would only land if we committed to a richer dev-vs-prod split via `tsconfig` `customConditions`, which Phase 14 explicitly punted), 14.2 was pure doc reconciliation, 14.3 is the issue's hypothesis option-1 wired through existing `metadata.questionId` plumbing, 14.4 is a small operator surface plus a README paragraph. `CompleteArchitecture.md` unchanged.
- **14.1 — Stale-dist dev-loop gotcha (overdue since Phase 9 close).** Every workspace-dep edit used to require a manual `pnpm build` before `pnpm factoryd` because `apps/factoryd` consumed workspace packages via `main: "./dist/index.js"`. Fix: each `packages/*/package.json` (15 files) gained a `"development": "./src/<entry>.ts"` condition in its `exports` map; root `pnpm factoryd` / `pnpm factory` and per-app `dev` scripts pass `--conditions=development` so dev runs route through src. Prod path (`node dist/main.js`) untouched — falls back to the `import` condition. Verified end-to-end on Windows + Node 22.22.2 + tsx 4.21.0: factoryd boots clean with `packages/daemon/dist/` renamed away (proves src routing); same boot fails with `ERR_MODULE_NOT_FOUND` when the flag is absent (proves the condition is what's wiring it up). Side effect (good): vitest's resolver activates `development` by default in test mode → tests now route to src too, so cross-package test imports no longer require a build.
- **14.2 — I013 status reconciliation.** Code re-read confirmed `prePurgeDepDirs` (rimrafs `node_modules` / `.venv` / `__pycache__` before `git worktree remove --force`) shipped in Phase 10.3 (`50bab61 feat(10.3)`); regression test at `worktree.test.ts:138` covers the node_modules-leak scenario; cross-runtime concern (`.venv` purged alongside) handled. Issue file moved to `status: RESOLVED, resolved: 2026-04-24`; INDEX row moved from Open to Resolved with the regression-test pointer. Pure doc reconciliation, no code change.
- **14.3 — I012 fix (Telegram Reply-feature matcher).** Took the issue's hypothesis option-1 ("record the bot's outbound message_id") and wired it through the existing `metadata.questionId` plumbing rather than a Telegram-specific send-path rewrite. Migration 008 adds `pending_questions.bot_message_id TEXT` (nullable) + a partial index on `(bot_message_id) WHERE bot_message_id IS NOT NULL AND answered_at IS NULL`. New `pendingQuestions.setBotMessageId` + `findOpenByBotMessageId` helpers (channel-scoped, idempotent). The outbound worker stamps the column after successful delivery when `msg.metadata.questionId` is a string — best-effort, a thrown DB error is logged but never fails the delivery. Telegram matcher gains a new exact rung: `findOpenByBotMessageId('telegram', String(replyTo.message_id))` runs first; falls through to the legacy `channel_ref` / `LIKE` rungs only when the exact rung misses (back-compat for pre-migration rows or outbounds whose delivery succeeded before the stamp). Discord untouched per Phase 7c live-data signal — its existing `discord.js`-based reply primitives don't show the FIFO mismatch. I012 → RESOLVED.
- **14.4 — `factory questions cleanup` + Windows mojibake README addendum.** New operator command for sweeping un-answered escalations / `ask_user` rows whose parent directive has already ended (terminal status — `complete` / `failed` / `blocked`). Surface: `factory questions cleanup [--since <iso-date>] [--dry-run]`. Lists each orphan oldest-first with directive/status/source/created-at; with `--dry-run` writes nothing; without, stamps each row's `answered_at` to `now()` and `answer` to a self-describing synthetic note (`[orphaned by factory questions cleanup at <when>: directive <id> ended <status>]`) — preserves forensic value while removing the row from the open-questions view. New `findOrphaned` / `markOrphanAnswered` helpers in `pendingQuestions`. Pure `runQuestionsCleanup` function exported for testability. Plus a "Windows operator tips" subsection in README.md pointing at `[Console]::OutputEncoding = [System.Text.Encoding]::UTF8` for PowerShell 5.1 em-dash mojibake (Windows Terminal + PowerShell 7+ usually pick this up automatically; classic 5.1 does not).
- Tests: **876** green across 15 packages (was 855). +21 from this phase: state +12 (5 from 14.3 `setBotMessageId` / `findOpenByBotMessageId` + 7 from 14.4 `findOrphaned` / `markOrphanAnswered`), channels +2 (14.3 telegram regressions), daemon +2 (14.3 outbound-worker), cli +5 (14.4 questions cleanup). `pnpm lint` + `pnpm format:check` clean. `pnpm build` clean across 15 packages + 3 apps.
- Migration count: **7 → 8** (migration 008 — `pending-questions-bot-message-id`). Idempotent re-apply tests in 003 / 004 / 006 bumped from `[1..7]` to `[1..8]`.
- **Issues moved to RESOLVED:** I012 (LOW, channels/telegram, resolved 2026-04-27 via the bot_message_id fix) + I013 (MEDIUM, worker/worktree, resolved 2026-04-24 via Phase 10.3, doc-status reconciled in 14.2). `docs/issues/INDEX.md` Open table is **empty for the first time** since the tracker was instituted.
- **Spend: $0** — all TS / docs / migration / IPC / CLI work; no live-LLM smoke runs needed.
- Phase 15 kicks off: **Demand-driven runoff** (indefinite duration; pending demand signal). Scaffolded in this close commit at `.control/phases/phase-15-demand-driven-runoff/`. The codebase is at a stable position with no concrete first sub-step; opens against whatever bites first. Candidate pool: Bash sandboxing (incident-driven, ADR 0028 §4 deferral), `/build` flag parsing on Telegram + Discord (operator-request-driven), network egress scoping (demand-driven), orphan `node.exe` on port 25295 investigation (noted during 14.1 smoke), Phase 6 operator follow-ups (out-of-band — PAT revoke, `gh repo delete`, env var cleanup).
- Carry-forward (still non-blocking, all in Phase 15's queue): Bash sandboxing, `/build` flag parsing, network egress scoping, orphan node.exe, Phase 6 operator follow-ups.
