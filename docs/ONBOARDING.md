...
| `/app/directives/detail/?id=<id>` | Inflight tasks, open pending questions, spend + call-count rollup for a single directive. SSE-live activity panel narrates brain stages as they happen (triage → architect → planner → pool → terminal). **Resume** pill on terminal directives (`failed` / `blocked` / `complete`, intent=build) re-enters the pipeline with the same project + wiki. See §5.4 + §5.5 |
| `/app/projects/`                  | Project registry list (most-recently-touched first). "Last build" column shows each project's latest build directive status linked to its detail; **Resume** pill in the same cell on terminal-non-running rows                                                                                                                                                        |
...
Detail pages stream live updates via SSE on `GET /api/v1/directives/:id/stream` — tasks transitioning, findings appearing, spend ticking up, and brain-stage `log.line` events (triage / architect / planner / pool / terminal) all render as they happen, no reload needed. The page subscribes on load (the connection backfills any events emitted before subscribe so connect-after-build is idempotent), maintains a 15 s `:keepalive` heartbeat, and falls back to polling on `/api/v1/directives/:id` for SSE-stripped proxies. ADR 0029 pins the protocol; ADR 0031 pins the brain-side emission convention (manual `emitLogLine` sites at every stage entry / exit / error path; error events carry the first 500 chars of any offending LLM output in `attrs.detail`).

The dashboard is fully write-capable today: kick off builds (`/app/build/`), resume failed builds (`/app/directives/detail` + `/app/projects/` per §5.5), claim new projects (`/app/projects/new/`), edit budget defaults (`/app/projects/detail`), close pending questions (`/app/questions/detail`), and chat with the brain (`/app/chat/`). All writes go through the same brain-side state package the CLI uses — a build started in the SPA is indistinguishable from one started via `factory build`. ADR 0027 covers the mutation surface (auth, idempotency, error envelope).

### 5.5 Resume from the dashboard

When a build fails / blocks / completes and you want to retry without losing the architect's work, the dashboard offers a one-click Resume on two surfaces:

- **Directive detail (`/app/directives/detail/?id=<id>`)** — a vermillion **Resume** pill in the title row, visible whenever the directive's status is terminal (`failed`, `blocked`, `complete`) AND its intent is `build`. Mutex with the Cancel pill — never both visible. One click POSTs to `POST /api/v1/directives/:id/resume`, the daemon mints a child directive with `parentDirectiveId` + `payload.resumeFrom` set (inheriting language / projectId / limits / autonomy from the prior), and the page navigates to the child's detail.
- **Projects index (`/app/projects/`)** — the "Last build" column carries the latest build directive's status linked to its detail; a small **Resume** pill appears in the same cell on terminal-non-running rows.

The brain handles the resume specifics: skips the architect when the wiki is on disk (look for `architect: skipped (wiki already on disk; resume path)` in the activity panel) and skips already-complete tasks in the plan. The CLI mirror is `factory resume <project>`. The daemon refuses with 409 if the prior directive is still `running`/`pending` (cancel it first), 422 if its `projectPath` is missing on disk, and 404 if the prior id doesn't exist.
