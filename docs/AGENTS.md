...
Agents are *roles* in the build pipeline. Each agent has:
...
Each agent declares a *category*, not a model. The provider layer resolves category → provider+model via the user's `~/.factory5/config.toml`. See ADR 0004.
